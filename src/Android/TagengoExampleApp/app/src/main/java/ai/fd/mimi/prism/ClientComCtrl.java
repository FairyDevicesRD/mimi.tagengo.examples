...
public class ClientComCtrl {
    ...
    private WebSocketRecognizer wsRecog = null;
    ...
    public ResponseData request(String url, String xmlData) throws IOException, SAXException {
        ...
        switch (request.type) {
            case SR:
                if (isTransferEncodingChunked()) {
                    wsRecog = new WebSocketRecognizer(accessToken);
                    RequestData requestData = util.parseXML(xmlData);
                    wsRecog.startRecognize(url, requestData);
                    return null;
                } else {
                    // SR一括送信用のメソッドではない
                    throw new IllegalStateException();
